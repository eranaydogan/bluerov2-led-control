...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Convert NED position delta into the robot's initial body frame "
            "before sending to Unity. Output convention: UDP x=lateral, y=forward, z=up."
        ),
    )

    parser.add_argument("--body-forward-sign", type=float, default=1.0)
    parser.add_argument("--body-lateral-sign", type=float, default=-1.0)
    parser.add_argument("--body-up-sign", type=float, default=1.0)
    ...
    print(f"body_lateral_sign  : {args.body_lateral_sign}")
    print(f"body_up_sign       : {args.body_up_sign}")
        ...
        while True:
            ...
            if latest_pos is not None and latest_att is not None:
                ...
                if args.body_frame_relative:
                    ...
                    #   yaw=+90 -> facing East
                    #
                    # First yaw is used as the initial body-frame reference.
                    # This removes the dependency on always starting exactly facing East.

                    yaw0 = origin_yaw
                    cos_yaw0 = math.cos(yaw0)
                    sin_yaw0 = math.sin(yaw0)

                    # World/NED delta projected onto initial body axes.
                    body_forward = cos_yaw0 * dx_n + sin_yaw0 * dy_e
                    body_right = -sin_yaw0 * dx_n + cos_yaw0 * dy_e
                    body_up = -dz_d

                    # Unity receiver synthetic axis probe showed:
                    #   UDP x +1 -> camera left
                    #   UDP y +1 -> camera forward
                    #   UDP z +1 -> camera up
                    #
                    # Therefore:
                    #   body_forward -> UDP y
                    #   body_right   -> UDP x, with default negative sign
                    #   body_up      -> UDP z
                    x_forward = args.body_lateral_sign * body_right
                    y_right = args.body_forward_sign * body_forward
                    z_up = args.body_up_sign * body_up
                ...
                if now - last_print >= (1.0 / args.print_rate):
                    ...
                    print(
                        f"seq={seq:06d} "
                        f"udp_xyz=({x_forward:+.3f}, {y_right:+.3f}, {z_up:+.3f}) "
